...
class TestTestMapper:
    ...
    def test_create_mappings_with_no_source_files_and_one_test_file_changed(self, repo_with_no_source_files_and_one_test_file_changed):
        revisions = [commit.hexsha for commit in repo_with_no_source_files_and_one_test_file_changed.iter_commits('master')]
        start_date = datetime.combine(datetime.now() - timedelta(days=1), time())
        project = "my_project"
        branch = "master"
        test_mappings = under_test.TestMapper.create_mappings(repo_with_no_source_files_and_one_test_file_changed, revisions, TEST_RE, SOURCE_RE, start_date, project, branch)
        test_mappings_list = test_mappings.get_mappings()
        assert len(test_mappings_list) == 0


    #  def test_create_mappings_with_one_source_file_and_one_test_file_changed_in_different_commits
